...
if __name__ == '__main__':
	...
	#flag = b'FLAG{abcdefghij}'
	encrypted = open('cipher', 'rb').read()
	for i in range(10):
		for sb in range(33,126):
			#flag = b"".join([flag[:5+i], bytes(sb), flag[6+i:]])
			flag = flag[:5+i]+bytes(chr(sb),'ascii')+flag[6+i:]
			#print('try flag: %s'%flag)
			p = encrypt(flag, 115)
			if encrypted[:6+i] == p[:6+i]:
				print("current flag:%s"%(flag[:6+i]))
				break
